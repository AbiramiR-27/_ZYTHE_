...
class ReflectListImpl2 {
  ...
  *entries() {
    for (let i2 = 0;i2 < this._arr.length; i2++) {
      yield [i2, listItemToReflect2(this._field, this._arr[i2], this.check)];
    }
  }
}
...
function getDecodeTable2() {
  if (!decodeTable2) {
    ...
    for (let i2 = 0;i2 < encodeTable.length; i2++)
      decodeTable2[encodeTable[i2].charCodeAt(0)] = i2;
...
function createBaseRegistry2() {
  ...
  return {
    ...
    add(desc) {
      ...
      types4.set(desc.typeName, desc);
    },
    get(typeName) {
      return types4.get(typeName);
    },
    getFile(fileName) {
      return files.get(fileName);
    },
    getMessage(typeName) {
      const t = types4.get(typeName);
      return (t === null || t === undefined ? undefined : t.kind) == "message" ? t : undefined;
    },
    getEnum(typeName) {
      const t = types4.get(typeName);
...
function setExtension3(message, extension, value2) {
  var _a;
  ...
  for (const [jsonKey, jsonValue] of Object.entries(json)) {
    ...
    if (field) {
      ...
      if (jsonKey.startsWith("[") && jsonKey.endsWith("]") && (extension = (_a = opts.registry) === null || _a === undefined ? undefined : _a.getExtension(jsonKey.substring(1, jsonKey.length - 1))) && extension.extendee.typeName === msg.desc.typeName) {
        ...
        setExtension3(msg.message, extension, get());
      }
      if (!extension && !opts.ignoreUnknownFields) {
        throw new Error(`cannot decode ${msg.desc} from JSON: key "${jsonKey}" is unknown`);
      }
...
var encodeCallMsg = (payload) => {
  const encodeField = (fieldName, value2) => {
    try {
      return hexToBase642(value2);
    } catch (e) {
...
var prepareReportRequest = (hexEncodedPayload, reportEncoder = EVM_DEFAULT_REPORT_ENCODER) => ({
  encodedPayload: hexToBase642(hexEncodedPayload),
  ...reportEncoder
});
...
(function(util4) {
  ...
  util4.jsonStringifyReplacer = (_, value3) => {
...
class ZodError3 extends Error {
  ...
  static assert(value3) {
    if (!(value3 instanceof ZodError3)) {
...
var DIRTY2 = (value3) => ({ status: "dirty", value: value3 });
var OK2 = (value3) => ({ status: "valid", value: value3 });
...
class ZodNumber2 extends ZodType2 {
  ...
  gte(value3, message) {
    return this.setLimit("min", value3, true, errorUtil2.toString(message));
  }
  gt(value3, message) {
    return this.setLimit("min", value3, false, errorUtil2.toString(message));
  }
  lte(value3, message) {
    return this.setLimit("max", value3, true, errorUtil2.toString(message));
  }
  lt(value3, message) {
    return this.setLimit("max", value3, false, errorUtil2.toString(message));
  }
...
class ZodObject2 extends ZodType2 {
  ...
  _parse(input) {
    ...
    for (const key of shapeKeys) {
      ...
      pairs.push({
        key: { status: "valid", value: key },
        value: keyValidator._parse(new ParseInputLazyPath2(ctx, value3, ctx.path, key)),
        alwaysSet: key in ctx.data
      });
    }
    if (this._def.catchall instanceof ZodNever2) {
      ...
      for (const key of extraKeys) {
        ...
        pairs.push({
          key: { status: "valid", value: key },
          value: catchall._parse(new ParseInputLazyPath2(ctx, value3, ctx.path, key)),
          alwaysSet: key in ctx.data
        });
      }
    }
  ...
  required(mask) {
    ...
    for (const key of util3.objectKeys(this.shape)) {
      if (mask && !mask[key]) {
        ...
        const fieldSchema = this.shape[key];
        let newField3 = fieldSchema;
        while (newField3 instanceof ZodOptional2) {
          newField3 = newField3._def.innerType;
        }
        newShape[key] = newField3;
      }
    }
    return new ZodObject2({
      ...this._def,
...
class ZodMap2 extends ZodType2 {
  ...
  _parse(input) {
    ...
    const keyType = this._def.keyType;
    const valueType = this._def.valueType;
    const pairs = [...ctx.data.entries()].map(([key, value3], index) => {
      return {
        key: keyType._parse(new ParseInputLazyPath2(ctx, key, ctx.path, [index, "key"])),
        value: valueType._parse(new ParseInputLazyPath2(ctx, value3, ctx.path, [index, "value"]))
      };
    });
    if (ctx.common.async) {
      const finalMap = new Map;
...
class ZythePredictionMarket {
  ...
  marketCounter(runtime3) {
    ...
    return decodeFunctionResult({
      abi: ZythePredictionMarketABI,
      functionName: "marketCounter",
      data: bytesToHex4(result.data)
    });
  }
  getMarket(runtime3, marketId) {
    const callData = encodeFunctionData({
      abi: ZythePredictionMarketABI,
  ...
  writeResolveMarket(runtime3, marketId, gasConfig) {
    const callData = encodeFunctionData({
      abi: ZythePredictionMarketABI,
    ...
    return this.client.writeReport(runtime3, {
      receiver: this.address,
      report: reportResponse,
      gasConfig
    }).result();
  }
  writeReport(runtime3, callData, gasConfig) {
    const reportResponse = runtime3.report(prepareReportRequest(callData)).result();
    return this.client.writeReport(runtime3, {
      receiver: this.address,
      report: reportResponse,
...
var getEvmClient = (config) => {
  ...
  return new cre.capabilities.EVMClient(network495.chainSelector.selector);
};
var onCronTrigger = (runtime3, payload) => {
  if (!payload.scheduledExecutionTime) {
    throw new Error("Scheduled execution time is required");
  }
  runtime3.log("Checking markets for resolution...");
  const evmClient = getEvmClient(runtime3.config);
  const contract = new ZythePredictionMarket(evmClient, runtime3.config.contractAddress);
  const count = contract.marketCounter(runtime3);
  runtime3.log(`Found ${count.toString()} total markets`);
  const now = BigInt(Math.floor(Date.now() / 1000));
  const resolved = [];
  const skipped = [];
  for (let i2 = 0n;i2 < count; i2++) {
    const market = contract.getMarket(runtime3, i2);
    if (market.resolved) {
      runtime3.log(`Market ${i2}: already resolved, skipping`);
      skipped.push(i2.toString());
      continue;
    }
    if (market.resolutionTime > now) {
      runtime3.log(`Market ${i2}: "${market.title}" deadline not yet reached (${new Date(Number(market.resolutionTime) * 1000).toISOString()}), skipping`);
      skipped.push(i2.toString());
      continue;
    }
    runtime3.log(`Market ${i2}: resolving "${market.title}" (strike: ${market.strikePrice.toString()}, deadline: ${new Date(Number(market.resolutionTime) * 1000).toISOString()})`);
    const resp = contract.writeResolveMarket(runtime3, i2, { gasLimit: runtime3.config.gasLimit });
    if (resp.txStatus !== TxStatus.SUCCESS) {
      throw new Error(`Failed to resolve market ${i2}: ${resp.errorMessage || resp.txStatus}`);
    }
    const txHash = bytesToHex(resp.txHash || new Uint8Array(32));
    runtime3.log(`Market ${i2}: resolved successfully, tx: ${txHash}`);
    resolved.push(i2.toString());
  }
  const summary = { resolved, skipped, total: Number(count) };
  runtime3.log(`Resolution complete: ${JSON.stringify(summary)}`);
